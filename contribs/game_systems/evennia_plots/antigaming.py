...
def _create_job(title, description):
    ...
    try:
        ...
        logger.exception("evennia_plots.antigaming: failed to create job for %r", title)
...
def _flag_thread_gaming(window_end):
    ...
    if flagged:
        logger.info("evennia_plots.antigaming: thread gaming - flagged %d thread(s)", flagged)
...
def sweep(window_end):
    ...
    """
    logger.info("evennia_plots.antigaming: sweep starting for window_end=%s", window_end)
    _flag_thread_gaming(window_end)
    logger.info("evennia_plots.antigaming: sweep complete")
