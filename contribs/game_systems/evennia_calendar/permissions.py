...
Uses Evennia's lock system (CALENDAR_STAFF_LOCK setting) rather than Django's
...
def _staff_lock_expr():
    ...
    lock = getattr(settings, "CALENDAR_STAFF_LOCK", "cmd:perm(Builder)")
    return lock[4:] if lock.startswith("cmd:") else lock


def is_staff_user(request) -> bool:
    """Return True if the request's account has calendar staff permissions.

    Uses Evennia's lock system (configured via CALENDAR_STAFF_LOCK, default
