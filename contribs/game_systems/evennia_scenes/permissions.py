...
"""
Permission helpers for evennia_scenes web views and API.

Uses Evennia's lock system (SCENES_STAFF_LOCK setting) rather than Django's
is_staff flag, which is unrelated to in-game permission levels.

Usage::

    from evennia_scenes.permissions import is_staff_user, get_character_id, require_character
...
def _staff_lock_expr():
    ...
    lock = getattr(settings, "SCENES_STAFF_LOCK", "cmd:perm(Builder)")
    return lock[4:] if lock.startswith("cmd:") else lock


def is_staff_user(request) -> bool:
    """Return True if the request's account has scenes staff permissions.

    Uses Evennia's lock system (configured via SCENES_STAFF_LOCK, default
